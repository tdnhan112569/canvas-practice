...
class PieChartViewClick2(context: Context, attrs: AttributeSet) : View(context, attrs) {
...
//        color = ContextCompat.getColor(context, R.color.xam_dam);
    ...
    private var rectOfNormalPie = RectF()
    private var rectOfSelectedPie = RectF()
    private var rectImageOuter = Rect()
    private var rectImageInner = Rect()
    ...
    private var spacingOuterWithSelectedPie = 12.dpToPx
    private var spacingSelectedPieWithNormalPie = 6.dpToPx
    private var sizeOfNormalPie = 36.dpToPx
    private var distanceBetweenNormalPieWithCenter = 0f

    private var distanceBetweenSelectedPieWithInnerImage = 2.dpToPx
    ...
    private fun onDrawPie(canvas: Canvas) {
        var degree = 0f
        var sweepDegree = 0f
        val drawableOuter = ContextCompat.getDrawable(context, R.drawable.ic_group_outter)
        val drawableInner = ContextCompat.getDrawable(context, R.drawable.ic_bg_inner)
        canvas.apply {

            drawableOuter?.apply {
                bounds = rectImageOuter
                draw(canvas)
            }

            drawArc(
                RectF(
                    rectImageInner.left.toFloat(),
                    rectImageInner.top.toFloat(),
                    rectImageInner.right.toFloat(),
                    rectImageInner.bottom.toFloat()
                ),
            ...
            for (type in listOfSpending) {
                ...
                type.isCurrent = currentDeg - 90f in startDeg..endDeg
                drawArc(rectOfNormalPie, type.fromDeg, sweepDegree, true, type.paint)
                startDeg = endDeg
            }
            drawCircle(centerPoint.first, centerPoint.second, distanceBetweenNormalPieWithCenter, circleWhitePaint)

            listOfSpending
                .firstOrNull { it.isCurrent }
                ?.let {
                    sweepDegree = it.toDeg - it.fromDeg
                    drawArc(rectOfSelectedPie, it.fromDeg, sweepDegree, true, it.paint)
            ...
            drawCircle(
                centerPoint.first,
                centerPoint.second,
                distanceBetweenNormalPieWithCenter - spacingSelectedPieWithNormalPie,
                circleWhitePaint
            )

            drawableInner?.apply {
                bounds = rectImageInner
                draw(canvas)
            }
    ...
    private fun initData() {
        ...
        val (x, y) = centerPoint
        val topLeft = Pair(
            x - halfSide + paddingStart,
            y - halfSide + paddingTop
        )
        val topRight = Pair(
            x + halfSide - paddingEnd,
            topLeft.second
        )
        val bottomLeft = Pair(
            topLeft.first,
            y + halfSide - paddingBottom
        )
        val bottomRight = Pair(topRight.first, bottomLeft.second)

        rectImageOuter = Rect(
            topLeft.first.roundToInt(),
            topLeft.second.roundToInt(),
            topRight.first.roundToInt(),
            bottomRight.second.roundToInt()
        )

        rectOfSelectedPie = Rect(
             rectImageOuter.left + spacingOuterWithSelectedPie,
             rectImageOuter.top + spacingOuterWithSelectedPie,
             rectImageOuter.right - spacingOuterWithSelectedPie,
             rectImageOuter.bottom - spacingOuterWithSelectedPie
        ).toRectF()

        rectOfNormalPie = RectF(
            rectOfSelectedPie.left + spacingSelectedPieWithNormalPie,
            rectOfSelectedPie.top + spacingSelectedPieWithNormalPie,
            rectOfSelectedPie.right - spacingSelectedPieWithNormalPie,
            rectOfSelectedPie.bottom - spacingSelectedPieWithNormalPie
        )

        distanceBetweenNormalPieWithCenter = Pair(
            rectOfNormalPie.left, centerPoint.second
        ).distance(centerPoint) - sizeOfNormalPie

        rectImageInner = Rect(
            (rectOfNormalPie.left + sizeOfNormalPie + spacingSelectedPieWithNormalPie + distanceBetweenSelectedPieWithInnerImage).roundToInt(),
            (rectOfNormalPie.top + sizeOfNormalPie + spacingSelectedPieWithNormalPie + distanceBetweenSelectedPieWithInnerImage).roundToInt(),
            (rectOfNormalPie.right - sizeOfNormalPie - spacingSelectedPieWithNormalPie - distanceBetweenSelectedPieWithInnerImage).roundToInt(),
            (rectOfNormalPie.bottom - sizeOfNormalPie - spacingSelectedPieWithNormalPie - distanceBetweenSelectedPieWithInnerImage).roundToInt()
        )
    ...
    private fun Pair<Float, Float>.distance (point: Pair<Float, Float>): Float {
        return abs(sqrt(
            (first - point.first).toDouble().pow(2) + (second - point.second).toDouble()
            .pow(2)
        )).toFloat()
    }
